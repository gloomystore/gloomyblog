...
export const getServerSideProps: GetServerSideProps<Props> = async (ctx: GetServerSidePropsContext<ParsedUrlQuery>) => {
  try {
    const { boardidx } = ctx.params || {};
    const res = await axios.get(process.env.NEXT_PUBLIC_API_URL + '/dummy/getBoardData.json')
    // UUID 값에 대한 추가 로직 수행
    ...
    } else throw new Error('get Board Data Failed')
  } catch(er) {
    console.log(er)
    return {
      props: {
...
export default function Home({
  boardData = [],
  boardidx = undefined,
}:{boardData:any[], boardidx: number | undefined}) {
  // 로딩
